...
two fused phases (see spec §2.2):
  ...
  1. Additive (extrapolation): h_add_k = MLP_add(h_{k-1})
...
ablation. There is no action modality in the MOVi dataset — the lens
refines a purely visual latent toward a physically-grounded target, so the
additive phase consumes only the previous latent.
"""
